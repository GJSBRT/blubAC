...
AddEventHandler("playerSpawned", function()
    Citizen.Wait(80000) -- Change this to a higher value if you get banned when entering the server
    if firstSpawn then
...
AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checknearbypeds", function()
    if blubAC.AntiPedRevive then
        ...
        if _target ~= -1 then
            ...
            local distance = #(blubAC.HospitalCoords - vector3(_pcoords))
            if distance < 50 then
...
AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checkifneargarage", function()
    if blubAC.AntiVehicleSpawn then
        local _pcoords = GetEntityCoords(PlayerPedId())
        local isneargarage = false
        for k,v in pairs(blubAC.GarageList) do
...
if blubAC.Enable then
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            local _ped = PlayerPedId()
            local _pid = PlayerId()
            if blubAC.GodModeProtection then
            ...
            Citizen.Wait(200)
            if blubAC.MainAnticheat then
                Citizen.Wait(50)
                if blubAC.AntiRagdoll then
                    if not CanPedRagdoll(_ped) then
                        if not IsPedInAnyVehicle(_ped, true) and not IsEntityDead(_ped) then
                            local closestVehicle, _distance = blubACGetClosestVehicle()
                ...
                if blubAC.AntiExplosiveBullets then
                ...
                Citizen.Wait(100)
                if blubAC.AntiFlyandVehicleBelowLimits then
            ...
            Citizen.Wait(200)
            if blubAC.AntiBlacklistedWeapons then
            ...
            Citizen.Wait(200)
            if blubAC.AntiThermalVision then
                if GetUsingseethrough() then
                    sendinfotoserver("Ue53dCG6hctHvrOaJB5Q", "thermalvision") -- BAN (THERMAL VISION)
                end
            end
            Citizen.Wait(200)
            if blubAC.AntiNightVision then
                if GetUsingnightvision() then
                    sendinfotoserver("Ue53dCG6hctHvrOaJB5Q", "nightvision") -- BAN (NIGHT VISION)
                end
            end
            Citizen.Wait(200)
            if blubAC.AntiResourceStartorStop then -- if you get banned when activating this, delete this lines
            ...
            Citizen.Wait(200)
            if blubAC.AntiLicenseClears then
            ...
            Citizen.Wait(200)
            if blubAC.DisableVehicleWeapons then
            ...
            Citizen.Wait(200)
            local V = {"/e", "/f", "/d"}
            if blubAC.AntiKeyboardNativeInjections then
            ...
            Citizen.Wait(200)
            if blubAC.AntiCheatEngine then
            ...
            Citizen.Wait(200)
            if blubAC.AntiCommandInjection then
    ...
    Citizen.CreateThread(function()
        while blubAC.AntiInfiniteAmmo do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
            if IsPedArmed(_ped, 6) then
                ...
                if blubAC.AntiDamageModifier then
    ...
    Citizen.CreateThread(function()
        while blubAC.AntiVehicleModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
    ...
    Citizen.CreateThread(function()
        while blubAC.CheckPlayersMoney do
            Citizen.Wait(5000)
            local efectivo = nil
            local banco = nil
    ...
    AddEventHandler("gameEventTriggered", function(name, args)
        if blubAC.DeleteBrokenCars then
    ...
    AddEventHandler('ZRQA3nmMqUBOIiKwH4I5:clearpeds', function()
        if blubAC.ClearPedsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearprops", function()
        if blubAC.ClearObjectsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearvehicles", function(vehicles)
        if vehicles == nil then
            ...
            if blubAC.ClearVehiclesAfterDetection then
                local vehs = GetGamePool('CVehicle')
                for _, vehicle in ipairs(vehs) do
    ...
    if blubAC.AntiResourceStartorStop then
        ...
        Citizen.CreateThread(function()
            ...
            while blubAC.AntiResourceStartorStop do
                Citizen.Wait(30000)
                for i = 0, nr do
    ...
    blubACGetClosestVehicle = function(coords)
        local vehicles        = blubACGetVehicles()
...
AddEventHandler("onClientResourceStart", function(res)
    ...
    if rlength >= blubAC.MaxResourceNameLength then -- Adjust this if you get banned while entering the server
